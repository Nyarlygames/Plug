...
public class CameraGO : MonoBehaviour {
	...
	void Update ()
    {
        ...
        if (((tribePos.position.x < GM.map.tilesizex /2 - thiscam.orthographicSize*2) && (tribePos.position.x > thiscam.orthographicSize*2))
            && ((tribePos.position.y < GM.map.tilesizey /2 - thiscam.orthographicSize) && (tribePos.position.y > thiscam.orthographicSize))) {
            temp = new Vector3(tribePos.position.x, tribePos.position.y, -thiscam.orthographicSize);
        }
        else if (tribePos.position.x >= GM.map.tilesizex / 2 - thiscam.orthographicSize * 2)
        {
            if (tribePos.position.y >= GM.map.tilesizey/2 - thiscam.orthographicSize)
                temp = new Vector3(GM.map.tilesizex / 2 - thiscam.orthographicSize * 2, GM.map.tilesizey / 2 - thiscam.orthographicSize, -thiscam.orthographicSize);
            else if (tribePos.position.y <= thiscam.orthographicSize)
                temp = new Vector3(GM.map.tilesizex / 2 - thiscam.orthographicSize * 2, thiscam.orthographicSize, -thiscam.orthographicSize);
            else
        ...
        else if (tribePos.position.y >= GM.map.tilesizey / 2 - thiscam.orthographicSize)
        {
            if (tribePos.position.x >= GM.map.tilesizex / 2 - thiscam.orthographicSize * 2)
                temp = new Vector3(GM.map.tilesizex / 2 - thiscam.orthographicSize * 2, GM.map.tilesizey / 2 - thiscam.orthographicSize, -thiscam.orthographicSize);
            else if (tribePos.position.x <= thiscam.orthographicSize * 2)
                temp = new Vector3(thiscam.orthographicSize * 2, GM.map.tilesizey / 2 - thiscam.orthographicSize, -thiscam.orthographicSize);
            else
